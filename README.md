...
- **Mention Trigger:** Set `trigger_mention` (or `--trigger-mention`) to a GitHub username so the daemon only reviews PRs where that account has been @-mentioned (e.g. commenting `@fiach-bot`), instead of every open PR. Each mention triggers one review — mention again to re-review. Only mentions from `allowed_mention_users` count (or, when unset, from commenters matching `allowed_author_associations`), and a trusted mention also lets the review run on PRs from authors outside the allowlist, so a maintainer can summon the bot on a new contributor's PR. A trusted mention also overrides the automatic backport exclusion; backports without a trusted mention remain excluded, and explicit `skip_prs` entries still take precedence. When the review starts, the triggering comment receives the `review_start_reaction` (default 👀) so the mentioner knows the bot saw it, and when it finishes with nothing actionable (no findings, all findings rejected by the verifier, or already reported) the 👀 is replaced with the `no_findings_reaction` (default 👍).
...
After the independent verifier finishes, Fiach fetches current PR discussion and uses TypeSafe Jev to compare verified findings with existing comments. Set `TYPESAFE_API_KEY` in the process environment or NixOS service environment file; it is forwarded into review sandboxes. Jev decisions take effect immediately, without shadow mode. The integration uses GDK’s `goose-providers` decision API with the direct TypeSafe provider and pins the model to `jev-1.13.0`. Goose and its provider crate are pinned together to upstream commit `8a460f964ef5b165e35ab231e76ccfa0c133abbd`, which includes decision-provider support. Fiach retains answer validation, caching, cost budgets, and shared provider cooldowns.
...
Lane selection captures the complete diff with three context lines per hunk, up to 80 KiB, plus up to 8 KiB of changed paths. Its serialized Jev request is capped at 96 KiB; deduplication requests retain a 24 KiB cap. These are byte limits, not token counts: Jev separately enforces its 32k-token state-plus-longest-question limit and 64k-token total request limit. API context-limit errors run all configured lanes. Diff-size failures and Git failures have distinct diagnostics, also recorded in lane-selection artifacts. Requests have a ten-second timeout and each Jev phase has a thirty-second total timeout. Inputs are never truncated to force a decision. Reported usage is included in review totals, using the pinned model's [published price](https://docs.typesafe.ai/models) of $0.042 per million input tokens (output is free). Estimated request cost is checked against the remaining budget before sending. Usage from requests that fail without a usable response cannot be accounted for; Fiach does not retry those requests before falling back.
