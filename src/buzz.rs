...
pub async fn publish_review_thread(
    ...
    if security_review && findings.is_empty() {
        tracing::info!(
            repo = %metadata.repository,
            pr = metadata.pr_number,
            "No verified security findings to publish to the private Buzz channel"
        );
        return Ok(None);
    }
    ...
    let root_id =
        EventId::from_hex(&root_event_id).context("Buzz returned an invalid root event id")?;

    let mut finding_event_ids = Vec::with_capacity(findings.len());
...
mod tests {
    ...
    use super::*;
    use crate::reporting::{AffectedLocation, CommandTranscript, Finding, Verdict};
